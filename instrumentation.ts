export async function register() {
  if (process.env.NEXT_RUNTIME === 'nodejs') {
    ...
    // Prefer IPv4 for outbound connections (Supabase Postgres, TMDB fetch(), etc).
    // Node resolves and tries IPv6 first by default; in Docker/dev-container
    // networking IPv6 routes often exist but don't actually work, so every
    // request/connection pays for a failed IPv6 attempt before falling back —
    // and sometimes that attempt hard-fails instead of falling back, surfacing
    // as "TypeError: fetch failed" / AggregateError / ETIMEDOUT. This makes
    // IPv4 the first (and effectively only) attempt.
